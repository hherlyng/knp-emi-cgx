...
class SolverEMI(object):
    ...
    def assemble_system(self):
        ...
        multiphenicsx.fem.petsc.assemble_matrix_block(
            self.A, p.a, bcs=p.bcs, restriction=(p.restriction, p.restriction)
        )  # Assemble DOLFINx matrix
        multiphenicsx.fem.petsc.assemble_vector_block(
            self.b, p.L, p.a, bcs=p.bcs, restriction=p.restriction
        )  # Assemble RHS vector
    ...
    def assemble_preconditioner(self):
        ...
        if not p.dirichlet_bcs:
            P_assembled = multiphenicsx.fem.petsc.assemble_matrix_block(
                p.P, bcs=[], restriction=(p.restriction, p.restriction)
            )
        else:
            P_assembled = multiphenicsx.fem.petsc.assemble_matrix_block(
                p.P, bcs=p.bcs, restriction=(p.restriction, p.restriction)
            )
    ...
    def setup_solver(self):
        ...
        self.A = multiphenicsx.fem.petsc.create_matrix_block(
            p.a, restriction=(p.restriction, p.restriction)
        )
        self.b = multiphenicsx.fem.petsc.create_vector_block(
            p.L, restriction=p.restriction
        )

        # Create solution vector
        self.x = multiphenicsx.fem.petsc.create_vector_block(
            p.L, restriction=p.restriction
        )
    ...
    def create_and_set_nullspace(self):
        ...
        ns_vec = multiphenicsx.fem.petsc.create_vector_block(
            p.L, restriction=p.restriction
        )
        ns_vec.array[:] = 1
        ns_vec.normalize()
    ...
    def solve(self):
        ...
        for i in range(self.time_steps):
            ...
            p.phi_M.x.array[:] = (
                p.u_p[0].x.array - p.u_p[1].x.array
            )  # Membrane potential
