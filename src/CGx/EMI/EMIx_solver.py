...
class SolverEMI(object):
    def __init__(
        ...
        self.direct_solver = (
            use_direct_solver  # Set direct solver/iterative solver option
        )
        ...
        if self.use_block_Jacobi:
            self.problem.setup_preconditioner()

        # Initialize output files
        if save_xdmfs:
            self.init_xdmf_savefile()
        if save_pngs:
            self.init_png_savefile()
    ...
    def assemble_system(self):
        ...
        # Assemble system
        assemble_matrix_block(self.A, p.a, bcs=p.bcs)
        self.A.assemble()
        assemble_vector_block(self.b, p.L, p.a, bcs=p.bcs)
    ...
    def assemble_rhs(self):
        ...
        assemble_vector_block(self.b, p.L, p.a, bcs=p.bcs)  # Assemble RHS vector
    ...
    def assemble_preconditioner(self):
        ...
        if not p.dirichlet_bcs:
            P_assembled = assemble_matrix_block(p.P, bcs=[])
        else:
            P_assembled = assemble_matrix_block(p.P, bcs=p.bcs)
    ...
    def setup_solver(self):
        ...
        self.A = create_matrix_block(p.a)
        self.b = create_vector_block(p.L)

        # Create solution vector
        self.x = create_vector_block(p.L)
    ...
    def create_and_set_nullspace(self):
        ...
        # Create a nullspace vector spanning the nullspace that is the set of constants
        ns_vec = create_vector_block(p.L)
        ns_vec.array[:] = 1
        ns_vec.normalize()
    ...
    def solve(self):
        ...
        for i in range(self.time_steps):
            ...
            if i == 0:
                ...
                if self.direct_solver:
                    ...
                    if not p.dirichlet_bcs:
                        ...
                        self.ksp.getPC().getFactorMatrix().setMumpsIcntl(
                            icntl=24, ival=1
                    ...
                    self.ksp.setOperators(
                        self.A, self.P_
                    ) if self.use_P_mat else self.ksp.setOperators(self.A)

                # Add contribution to setup time
                setup_timer += self.comm.allreduce(
                    time.perf_counter() - tic, op=MPI.MAX
                )
            ...
            wh[0].x.array[:] = x.array[:p.num_dofs_Vi]
            wh[1].x.array[:] = x.array[p.num_dofs_Vi:]
            ...
            p.phi_M = p.u_p[0](p.i_res) - p.u_p[1](p.e_res)  # Membrane potential
            p.phi_M_prev.x.array[:] = 0.0#p.u_p[0].x.array.copy() - p.u_p[1].x.array.copy()
    ...
    def print_info(self):
        ...
        num_dofs = p.Vi.dofmap.index_map.size_local + p.Ve.dofmap.index_map.size_local
        ...
        num_cells = (
            p.mesh.geometry.dofmap.__len__()
            - p.mesh.topology.interprocess_facets().__len__()
        )
        ...
        if not self.direct_solver:
            print("System size = ", self.A.size[0])
        ...
        if self.direct_solver:
            ...
            if self.use_P_mat:
                print("Preconditioning enabled.")

            print(
                "Average iterations: "
                + str(sum(self.iterations) / len(self.iterations))
            )
    ...
    def init_png_savefile(self):
        ...
        facets_gamma = p.boundaries.indices[
            gamma_indices
        ]  # The facets that lie on gamma
        dofs_gamma = dfx.fem.locate_dofs_topological(
            phi_M_space, p.mesh.topology.dim - 1, facets_gamma
        )  # The dofs of the gamma facets
        self.point_to_plot = dofs_gamma[
            int(len(dofs_gamma) / 2)
        ]  # Choose one of the dofs as the point for plotting the membrane potential

        self.v_t = []
        self.v_t.append(1000 * p.phi_M_prev.x.array[self.point_to_plot])  # Converted to mV
        print(1000 * p.phi_M_prev.x.array[self.point_to_plot])
    ...
    def print_figures(self):
        ...
        if hasattr(self.problem, "n"):
            plt.figure(1)
            plt.plot(
                np.linspace(0, 1000 * time_steps * dt, time_steps + 1),
                self.n_t,
                label="n",
            )
            plt.plot(
    ...
    def init_xdmf_savefile(self):
        ...
        self.xdmf_file.write_function(p.u_p[1], float(p.t.value))
    ...
    def save_xdmf(self):
        ...
        self.xdmf_file.write_function(self.problem.u_p[0], float(self.problem.t.value))
        self.xdmf_file.write_function(self.problem.u_p[1], float(self.problem.t.value))
