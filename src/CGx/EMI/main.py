...
def main_yaml(yaml_file="config.yaml"):
    ...
    dxi = ufl.Measure("dx", domain=solver.problem.intra_mesh)
    dxe = ufl.Measure("dx", domain=solver.problem.extra_mesh)

    phi_i_L2_local = dfx.fem.assemble_scalar(dfx.fem.form(ufl.inner(phi_i, phi_i) * dxi))
    phi_i_L2_global = solver.comm.allreduce(phi_i_L2_local, op=MPI.SUM)
    phi_i_L2_global = np.sqrt(phi_i_L2_global)

    phi_e_L2_local = dfx.fem.assemble_scalar(dfx.fem.form(ufl.inner(phi_e, phi_e) * dxe))
