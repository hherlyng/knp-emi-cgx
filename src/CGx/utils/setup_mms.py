...
class SetupMMS:
    ...
    def get_MMS_terms_EMI_2D(self, time):
        ...
        class ExtracellularSource:
            ...
            def __call__(
                ...
                return (
                    8 * np.pi**2 * np.sin(2 * np.pi * x[0]) * np.sin(2 * np.pi * x[1])
                )
        ...
        [
            exact_functions[i].interpolate(expressions[i]) for i in range(4)
        ]  # Interpolate function expressions

        # Gather expressions
        # exact solutions
        exact_sols = {"phi_i": exact_functions[0], "phi_e": exact_functions[1]}
        # source terms
        src_terms = {"f_i": exact_functions[2], "f_e": exact_functions[3]}
    ...
    def get_exact_solution_KNPEMI(self):
        ...
        if self.dim == 2:
            ...
            Na_i_e = 0.7 + 0.3 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.exp(
                -t
            )
            Na_e_e = 1.0 + 0.6 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.exp(
                -t
            )
            # potassium (K) concentration
            K_i_e = 0.3 + 0.3 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.exp(
                -t
            )
            K_e_e = 1.0 + 0.2 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.exp(
                -t
            )
            # chloride (Cl) concentration
            Cl_i_e = 1.0 + 0.6 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.exp(
                -t
            )
            Cl_e_e = 2.0 + 0.8 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.exp(
                -t
            )
            # potentials
            ...
            z = self.z
            # sodium (Na) concentration
            Na_i_e = 0.7 + 0.3 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.sin(
                2 * np.pi * z
            ) * sp.exp(-t)
            Na_e_e = 1.0 + 0.6 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.sin(
                2 * np.pi * z
            ) * sp.exp(-t)
            # potassium (K) concentration
            K_i_e = 0.3 + 0.3 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.sin(
                2 * np.pi * z
            ) * sp.exp(-t)
            K_e_e = 1.0 + 0.2 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.sin(
                2 * np.pi * z
            ) * sp.exp(-t)
            # chloride (Cl) concentration
            Cl_i_e = 1.0 + 0.6 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.sin(
                2 * np.pi * z
            ) * sp.exp(-t)
            Cl_e_e = 2.0 + 0.8 * sp.sin(2 * np.pi * x) * sp.sin(2 * np.pi * y) * sp.sin(
                2 * np.pi * z
            ) * sp.exp(-t)
            # potentials
            phi_i_e = (
            ...
            phi_e_e = (
                sp.cos(2 * np.pi * x) * sp.cos(2 * np.pi * y) * sp.cos(2 * np.pi * z)
            )
    ...
    def get_MMS_terms_KNPEMI_2D(self, time):
        ...
        f_phi_i = -(
            (sp.diff(J_Na_i[0], x) + sp.diff(J_Na_i[1], y))
            + (sp.diff(J_K_i[0], x) + sp.diff(J_K_i[1], y))
            - (sp.diff(J_Cl_i[0], x) + sp.diff(J_Cl_i[1], y))
        )
        f_phi_e = -(
            (sp.diff(J_Na_e[0], x) + sp.diff(J_Na_e[1], y))
        ...
        Nai_e, Nae_e, Ki_e, Ke_e, Cli_e, Cle_e, phii_e, phie_e, phiM_e = [
            var_dfx_funcs[i] for i in range(len(var_dfx_funcs))
        ]
        ...
        f_Nai, f_Nae, f_Ki, f_Ke, f_Cli, f_Cle, f_phii, f_phie = [
            source_dfx_funcs[i] for i in range(len(source_dfx_funcs))
        ]
        ...
        init_Nai, init_Nae, init_Ki, init_Ke, init_Cli, init_Cle, init_phiM = [
            init_dfx_funcs[i] for i in range(len(init_dfx_funcs))
        ]
    ...
    def get_MMS_terms_KNPEMI_3D(self, time):
        ...
        # equations for potentials: fE = - F sum(z_k*div(J_k_r)
        f_phi_i = -(
            (sp.diff(J_Na_i[0], x) + sp.diff(J_Na_i[1], y) + sp.diff(J_Na_i[2], z))
            + (sp.diff(J_K_i[0], x) + sp.diff(J_K_i[1], y) + sp.diff(J_K_i[2], z))
            - (sp.diff(J_Cl_i[0], x) + sp.diff(J_Cl_i[1], y) + sp.diff(J_Cl_i[2], z))
        )
        f_phi_e = -(
            (sp.diff(J_Na_e[0], x) + sp.diff(J_Na_e[1], y) + sp.diff(J_Na_e[2], z))
