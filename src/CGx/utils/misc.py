...
def mark_subdomains_square(mesh: dfx.mesh.Mesh) -> dfx.mesh.MeshTags:
    ...
    def inside(x: numpy.typing.NDArray[np.float64]) -> numpy.typing.NDArray[np.bool_]:
        ...
        bool1 = np.logical_and(
            x[0] <= 0.75, x[0] >= 0.25
        )  # True if inside inner box in x range
        bool2 = np.logical_and(
            x[1] <= 0.75, x[1] >= 0.25
        )  # True if inside inner box in y range
    ...
    cell_tags = dfx.mesh.meshtags(
        mesh, cell_dim, np.arange(num_cells, dtype=np.int32), cell_marker
    )
...
def mark_subdomains_cube(mesh: dfx.mesh.Mesh) -> dfx.mesh.MeshTags:
    ...
    def inside(x: numpy.typing.NDArray[np.float64]) -> numpy.typing.NDArray[np.bool_]:
        ...
        bool3 = np.logical_and(
            x[2] <= 0.75, x[2] >= 0.25
        )  # True if inside inner box in z range
