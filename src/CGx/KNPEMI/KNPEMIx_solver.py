...
class SolverKNPEMI(object):
    ...
    def create_and_set_nullspace(self):
        ...
        with multiphenicsx.fem.petsc.BlockVecSubVectorWrapper(
            ns_vec, [p.W[0].dofmap, p.W[1].dofmap], p.restriction
        ) as C_dd_wrapper:
    ...
    def init_png_savefile(self):
        ...
        self.v_t = []
        self.v_t.append(
            1000 * p.phi_M_prev.x.array[self.point_to_plot]
        )  # Converted to mV
    ...
    def save_xdmf(self):
        ...
        for idx in range(self.problem.N_ions + 1):
            self.xdmf_file.write_function(
                self.problem.u_p[0].sub(idx), float(self.problem.t.value)
            )
            self.xdmf_file.write_function(
                self.problem.u_p[1].sub(idx), float(self.problem.t.value)
            )
